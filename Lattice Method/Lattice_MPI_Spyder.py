...
def main(pos,Nsteps,dt,epsilon,BoxSize,DIM):
    ...
    for k in range(Nsteps):
        ...
        print(pos,"1 ",ID)
        print(pos,"2", ID)
        acc = Compute_forces_2.Compute_Local_Forces(pos, epsilon, BoxSize, DIM, N, Rcutoff, sigma)
        CORNERS = Send_Positions.send_positions_near_sides(comm, pos, ID, x_range, y_range, Rcutoff, mod_x,mod_y)
        acc = Compute_forces_2.Compute_Non_Local_Forces(acc,pos,CORNERS,epsilon,BoxSize,DIM,N,Rcutoff,sigma,x_range,y_range,ID)
        vel = vel + 1/2*dt*acc
    ...
    if rank ==0:
       ...
       # plt.title("N = {} sigma = {}".format(N_particles,sigma))
        ...
        ani.save("videos/{}_new_modulo_{}_6_cores_2.mp4".format(N_particles,nr_proc))
    return acc,E
